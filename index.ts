...
function DarAssendangoogle(json: GoogleSheet, cardnum = 10) {
  let num: number = json.value.length,
    i;
  let title: string, subtitle: string, image_url: string;
  let manychat: ManychatGallery = {
    version: "v2",
...
function DarAssendansharepoint(
  json: SharePointListItems,
  cardnum = 10
): ManychatGallery {
  let num: number = json["value"].length,
    i: number;
  let title: string, subtitle: string, image_url: string;
  const manychat: ManychatGallery = {
    version: "v2",
    content: { messages: [], actions: [], quick_replies: [] },
  };
  for (i = 0; i < Math.floor(num / cardnum); i++) {
